...
export async function POST(req: NextRequest) {
  try {
    ...
    const payload = {
      ...
      messages: [
        { role: "system", content: "你是一个专门评判表情包搞笑程度的AI助手，你需要给出0-100的分数，越搞笑分数越高。你只需要返回数字。" },
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: "大模型API调用失败", detail: err }, { status: 500 });
    }
    ...
    if (
      ...
      if (match) {
        funniness = parseInt(match[0], 10);
      }
    }

    return NextResponse.json({ funniness });
  } catch (error) {
    return NextResponse.json({ error: "服务端异常", detail: String(error) }, { status: 500 });
  }
}